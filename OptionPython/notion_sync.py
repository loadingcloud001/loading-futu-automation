            ...
            try:
                ...
                if ivc > 0:
                    cp = plan_trade(stock, stock_price, round(stock_price, -1) if stock_price > 50 else round(stock_price, 0),
                                    iv_call=ivc, iv_put=ivp, is_call=True, risk_amount=182.41, profit_target_pct=None, stop_loss_pct=None, days_to_expiry=7)
                    call_plan = cp
                # Generate Put plan
                if ivp > 0:
                    pp = plan_trade(stock, stock_price, round(stock_price, -1) if stock_price > 50 else round(stock_price, 0),
                                    iv_call=ivc, iv_put=ivp, is_call=False, risk_amount=182.41, profit_target_pct=None, stop_loss_pct=None, days_to_expiry=7)
                    put_plan = pp
            except Exception:
                pass
...
def sync_daily_snapshot(metrics: list, log_fn=None) -> int:
    ...
    for m in metrics:
        ...
        props = {
            ...
            "My Target Price": number_val(0),
            "My Stop Price": number_val(0),
